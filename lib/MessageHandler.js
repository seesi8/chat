...
import { compressImage, deleteStorage, downloadText, formatDate, generateOPK, getRecentOPK, nextPow2, readFileBytes, uploadText, withinDistance } from "./functions";
...
export class MessageHandler {
    ...
    async decryptDmMessage(currentMessage, thread, id) {
        ...
        if (MessageHandler.isFileType(header.type)) {
            if (await getStoredFile(this.threadId, currentMessage.id) === undefined) {
                ...
                console.log(currentMessage.message, "apple")
            }
        }
    ...
    async x3dh(user_a, data_a, data_b, threadId, uid_b) {
        ...
        const verified = await checkKeySignature(publicKeySK, ub64(SPKBundle.SPKPublicKey), ub64(SPKBundle.signature))
        if (!verified) {
    ...
    async sendDRDMWithDHExchange(
        ...
        const paddingAmount = nextPow2(message.byteLength + 16) - message.byteLength - 16
        console.log(message.byteLength + 16, nextPow2(message.byteLength + 16))
