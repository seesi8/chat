...
export class MessageHandler {
    ...
    async decryptDmMessage(currentMessage, thread, id) {
        ...
        const headerKey = await getStoredKey(`headerKey_${this.threadId}_r`)

        const header = await decryptHeader(headerKey, currentMessage.header.ciphertext, currentMessage.header.nonce)
        ...
        if (decryptedMessage !== undefined) {
            if (!alreadyDecrypted && (currentMessage.type != MessageHandler.MESSAGETYPES.READ) && MessageHandler.isGroupableType(currentMessage.type)) {

                const readData = {
                    id: currentMessage.id,
                    timeRead: (new Date()).getTime(),
                }
                await this.sendMessageWithoutLock(JSON.stringify(readData), MessageHandler.MESSAGETYPES.READ, { timeSent: currentMessage.timeSent.toDate().getTime() });
            }
            if (MessageHandler.isTextType(currentMessage.type)) {
    ...
    async x3dh_r(user_a, data_a, data_b, salt, threadId) {
        ...
        await storeKey(nextHeaderKey, `nextHeaderKey_${threadId}_r`);
        await storeKey(expanded["chainKey2"], `nextHeaderKey_${threadId}_r_raw`);
        await storeKey(headerKey, `headerKey_${threadId}_r`);
        await storeKey(expanded["chainKey3"], `headerKey_${threadId}_r_raw`);
    ...
    async cycleDHRachet(message, n) {
        ...
        if (key_n == null || message.header.publicKey_n != await exportKey(key_n)) {
            ...
            const currentHeaderKey = await getStoredKey(`nextHeaderKey_${this.threadId}_r`)
            const currentHeaderKey_raw = await getStoredKey(`nextHeaderKey_${this.threadId}_r_raw`)
    ...
    async uploadMessage(chainKey, n, pn, publicKey_n, message, type = MessageHandler.MESSAGETYPES.TEXT, options = {}) {
        const messageId = uuidv4();
    ...
    async sendDRDM(message, n = 0, pn = 0, type = MessageHandler.MESSAGETYPES.TEXT, options = {}) {
        ...
        const { nextChainKey, messageId, encrypted } = await this.uploadMessage(chainKey, n, pn, publicKey_n, message, type, options)
        await storeKey(nextChainKey, `chainKey_${this.threadId}_s`);
        await storeKey(encrypted["nextChainKey"], `chainKey_${this.threadId}_s_raw`);
        await storeMessage(this.threadId, messageId, message);
    ...
    async sendDRDMWithDHExchange(
        ...
        const { nextChainKey, messageId, encrypted } = await this.uploadMessage(chainKey_s, 0, pn, publicKey, message, type, options);
        ...
        await storeKey(nextHeaderKey_s, `nextHeaderKey_${threadId}_s`);
        await storeKey(nextHeaderKey_s_raw, `nextHeaderKey_${threadId}_s_raw`);
        await storeKey(currentHeaderKey, `headerKey_${threadId}_s`);
        await storeKey(currentHeaderKey_raw, `headerKey_${threadId}_s_raw`);
        await storeKey(privateKey, `privateKey_${this.threadId}_n`);
        await storeKey(publicKey, `publicKey_${this.threadId}_n`);
    ...
    async sendFileWithDHExchange(
        ...
        const currentHeaderKey_raw = await getStoredKey(`nextHeaderKey_${this.threadId}_s_raw`)
        await storeKey(nextHeaderKey, `nextHeaderKey_${threadId}_s`);
        await storeKey(nextHeaderKey_raw, `nextHeaderKey_${threadId}_s_raw`);
        await storeKey(currentHeaderKey, `headerKey_${threadId}_s`);
        await storeKey(currentHeaderKey_raw, `headerKey_${threadId}_s_raw`);
        await storeKey(privateKey, `privateKey_${this.threadId}_n`);
        await storeKey(publicKey, `publicKey_${this.threadId}_n`);
