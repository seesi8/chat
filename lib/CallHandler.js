...
import { getStoredFile, getStoredHeader, getStoredMessage } from "./e2ee/e2ee";
import { firestore } from "./firebase";
import { MessageHandler } from "./MessageHandler";
import { uuidv4 } from "@firebase/util";
import { doc, getDoc, onSnapshot } from "firebase/firestore";
import { stableStringify } from "./functions";
...
export class CallHandler extends MessageHandler {
    ...
    getCallState(finalMessages = this.decryptedMessages) {
        ...
        if (openCallRequest && openCallRequest.sentBy.id == this.user.uid) return CallHandler.CALLSTATES.CALL_REQUESTED;
        if (openCallRequest && openCallRequest.sentBy.id != this.user.uid) return CallHandler.CALLSTATES.INCOMING_CALL;
    ...
    createPC() {
        ...
        this.pc.onicecandidate = async (event) => {
            if (event.candidate) {
                ...
                await this.sendTypeWithLock(
                    stableStringify(event.candidate.toJSON()),
    ...
    async _decryptDmMessages(messagesValue) {
        ...
        for (let messageIndex in currentMessages) {
            const processed = await this._processSingleMessage(
                currentMessages,
                messageIndex,
                finalMessages,
                thread,
    ...
    async _decryptDmMessages(messagesValue) {
        ...
        for (let messageIndex in currentMessages) {
            ...
            if (await getStoredHeader(this.threadId, currentMessage.id) === undefined) {
