...
export async function generateAndStoreX25519Keypair(
  ...
  const db = await new Promise((resolve, reject) => {
    const req = indexedDB.open("crypto-keys", VERSION);
    req.onupgradeneeded = () => {
      const db = req.result;
...
export async function importHKDFKeyRaw(raw, usages = ["deriveBits"], extractable) {
...
function openCryptoDBWithOPKs() {
  return new Promise((resolve, reject) => {
    // First open with default version
    const req = indexedDB.open("crypto-keys");

    req.onupgradeneeded = (event) => {
      const db = event.target.result;
      if (!db.objectStoreNames.contains("opks")) {
        db.createObjectStore("opks");
      }
    };
    ...
    req.onsuccess = () => {
      ...
      // If opks already exists, we're done
      if (db.objectStoreNames.contains("opks")) {
        resolve(db);
        return;
      }

      // If DB exists but opks doesn't, bump version and create it
      const newVersion = db.version + 1;
      db.close();

      const req2 = indexedDB.open("crypto-keys", newVersion);
      req2.onupgradeneeded = (event2) => {
        const db2 = event2.target.result;
        if (!db2.objectStoreNames.contains("opks")) {
          db2.createObjectStore("opks");
        }
      };
