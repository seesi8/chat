...
export async function encryptSingleKey(my_priv, to_public, salt, key) {
    const sharedBits = await crypto.subtle.deriveBits(
        { name: "X25519", public: to_public },
        my_priv,
        256
...
export async function encryptKeysForMembers(my_priv, members, key) {
    const keys = {};

    for (const member of members) {
        const toPublicKey = await importX25519PublicRaw(ub64(member.publicKey));
        const salt = crypto.getRandomValues(new Uint8Array(16));
        const encryption = await encryptSingleKey(my_priv, toPublicKey, salt, key);
        keys[member.uid] = encryption;
    }
