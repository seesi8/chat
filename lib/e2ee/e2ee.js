...
export async function createDRDM(user, data) {
  const perm_priv = data.privateKey
  const other_perm_public = await importX25519PublicRaw(ub64(data.publicKey))

  const sharedBits = await crypto.subtle.deriveBits(
    { name: "X25519", public: other_perm_public },
    perm_priv,
    256
  );
