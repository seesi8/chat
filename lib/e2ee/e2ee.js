...
export async function encryptMLS(key, nonce, messageBytes, aad) {
  ...
  const ctBuf = await crypto.subtle.encrypt(
    { name: "AES-GCM", nonce, additionalData: te.encode(aad) },
    key,
    message
  );
...
export async function hkdfExpandWithLabels(input, info, size = 32) {
  info = te.encode(info);
...
export async function generateKeyPackage(privateIk, publicIk, index, uid) {
  ...
  const signature = await signKey(privateIk, te.encode(JSON.stringify(keypackage)))
