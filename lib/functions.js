...
import { GroupMessageHandler } from "./GroupMessageHandler.ts";

// FRIEND MANAGMENT

export async function removeFriend(friendId, user, userData) {
  await deleteKey(friendId)
...
export async function acceptFriend(id, user, data) {
  ...
  const messageHandler = new MessageHandler(user, data)

  if (user.uid > id) {
    await messageHandler.createDRDM([{ uid: user.uid }, { uid: id }], `${user.uid}${id}`, true)
  } else {
    await messageHandler.createDRDM([{ uid: user.uid }, { uid: id }], `${id}${user.uid}`, true)
...
export async function inviteFriend(friendId, user, data, threadId, messageHandler = null) {
  // let messagesName;
  // if (user.uid > friendId) {
  //   messagesName = `${user.uid}${friendId}`
  // } else {
  //   messagesName = `${friendId}${user.uid}`
  // }
  // const theDoc = (await getDocs(query(
  ...
  // let docData = theDoc.data()
  // docData.id = theDoc.id

  // const messageHandler = new MessageHandler(user, data, docData.id)
  // await messageHandler.sendTypeWithLock(`${threadId}`, MessageHandler.MESSAGETYPES.GROUP_INVITE)
  // await updateDoc(doc(firestore, "threads", threadId), {
  //   members: [
  //     ...members,
  //     friendId
  //   ]
  ...
  const handler = messageHandler ?? await GroupMessageHandler.create(user, data, threadId)
  await handler.addUser(friendId)
  return handler;
}
...
export async function rotate(user, data,threadId){
  const groupMessageHandler = await GroupMessageHandler.create(user, data, threadId)
  groupMessageHandler.startUpdate(false); 
}
...
export async function createUser(
  ...
  try {
    ...
    await generateKeyPackages({ uid: userUID }, {publicKeySK: SPKBundle.SPKPublicKey}, 10)
...
export async function generateAndStoreSupplementalKeyPairs(privateKey) {
  ...
  const signature = await sign(privateKey, publicRaw)
  const SPKBundle = {
...
export function mlsChildren(depthEdges, x) {
  if (!Number.isInteger(depthEdges) || depthEdges < 0) throw new Error("depthEdges must be a nonnegative int");
  if (!Number.isInteger(x) || x < 0) throw new Error("x must be a nonnegative int");

  const maxIndex = (1 << (depthEdges + 1)) - 2;
  if (x > maxIndex) return { left: null, right: null };

  const lvl = trailingOnes(x);
  if (lvl === 0) return { left: null, right: null };
  if (lvl > depthEdges) return { left: null, right: null };

  const offset = 1 << (lvl - 1);
  const left = x - offset;
  const right = x + offset;

  if (left < 0 || right > maxIndex) return { left: null, right: null };

  return { left, right };
}

export function depthEdgesFromLeaves(L) {
  if (!Number.isInteger(L) || L < 1) throw new Error("L must be an integer >= 1");
  if (L === 1) return 0;
  return Math.ceil(Math.log2(L));
}

export function stableStringify(value, opts = {}) {
  const {
    keyComparator = (a, b) => (a < b ? -1 : a > b ? 1 : 0),
    space,
    cycle = "throw",
  } = opts;
  ...
  function normalize(x) {
    // Match JSON.stringify behavior: call toJSON if present
    if (x && typeof x === "object" && typeof x.toJSON === "function") {
      x = x.toJSON();
    }
    ...
    const t = typeof x;
    if (t === "number") return Number.isFinite(x) ? x : null; // JSON.stringify -> null in arrays/objects
    ...
    if (t !== "object") {
      ...
      // We'll handle omission at object-walk time by returning a sentinel.
      return OMIT;
    }

    if (seen.has(x)) {
      if (cycle === "throw") throw new TypeError("Converting circular structure to JSON");
    ...
    if (Array.isArray(x)) {
      const arr = x.map((v) => {
        ...
        return nv === OMIT ? null : nv;
      });
      seen.delete(x);
      return arr;
    }

    const keys = Object.keys(x).sort(keyComparator);
    const obj = {};
    for (const k of keys) {
      const nv = normalize(x[k]);
      if (nv !== OMIT) obj[k] = nv;
    }
  ...
  // Top-level undefined/function/symbol becomes undefined -> JSON.stringify returns undefined
...
export async function createBackup(user, data, passphrase) {
  ...
  const encrypted = await encryptObject(
    stableStringify({ threadsData, SPKPrivateKey, privateKey, exportedOpks, mks: new_mks }),
    passphrase
  );
  return encrypted;
}
...
export async function generateKeyPackages(user, data, n = 10) {
  ...
  for (let index = 0; index < n; index++) {
    const keyPackage = await generateKeyPackage(privateIk, data.publicKeySK, index, user.uid)
...
export async function removeGroupMember(user, data, threadId, id){
  const groupMessageHandler = await GroupMessageHandler.create(user, data, threadId)
  
  await groupMessageHandler.removeUser(id)
