...
const bypassMissed = async (n, pn, storedN, threadId, currentMessage) => {
  if (n == 0 && pn > storedN) {
    for (let i = 0; i < pn - storedN; i++) {
      ...
      await bypassMissedMessage(threadId, storedN + i, public_key_old);
    }
  }
  if (n > storedN) {
    console.log("Missed Messages")
    for (let i = 0; i < n - storedN; i++) {
      await bypassMissedMessage(threadId, storedN + i, currentMessage.header.publicKey_n);
    }
  }
}
...
const decryptDmMessage = async (currentMessage, user, data, threadId, thread, id) => {
  ...
  if (decryptedMessage == undefined) {
    await bypassMissed(n, pn, storedN, threadId, currentMessage);
    if (n > storedN) storedN = n;
  }
  ...
  try {
    ...
    const batch = writeBatch(firestore);
    const ekPrivate = await getStoredKey(`ekPrivate_${threadId}`)
    const ekPublic = await importX25519PublicRaw(ub64(thread.ekPublic));
    sendWelcomeMessage(user, data, threadId, batch, ekPublic, ekPrivate)
    await batch.commit()
    console.log("Error in decrypting DRDM message, sent welcome message");
  }
...
export const decryptDmMessages = async (
  ...
  for (let messageIndex in currentMessages) {
    ...
    const decryptedMessage = await decryptDmMessage(currentMessage, user, data, threadId, thread, id);
...
export async function decryptDRDM(user, data, message, threadId, n, pn) {
  const chainKey = await getStoredKey(`chainKey_${threadId}_r`);
...
export async function bypassMissedMessage(threadId, n, publicKey_n) {
  ...
  if (!mk) {
    ...
    await storeMK(mk, `mk_${threadId}_${n}_${publicKey_n}`);
    await storeMK(mkBytes, `mk_${threadId}_${n}_${publicKey_n}_raw`);
  }
}
...
async function cycleDHRachet(user, data, message, threadId, n, currentMessage) {
  const key_n = await getStoredKey(`otherPublicKey_n_${threadId}`);
  if (key_n == null || currentMessage.header.publicKey_n != await exportKey(key_n)) {
    ...
    await storeMetadata(ns, `PN_${threadId}`);
    ...
    if (n > 0) {
      for (let i = 0; i < n - 0; i++) {
        await bypassMissedMessage(threadId, 0 + i, currentMessage.header.publicKey_n);
      }
    }
  }
}
...
export async function restoreBackup(userData, passphrase, setPopup) {
  ...
  try {
    ...
    console.log(loaded["exportedOpks"])
