  ...
  for (let messageIndex in currentMessages) {
    ...
    if (decryptedMessage == undefined) {
      if (
        ...
        decryptedMessage = await decryptDRDM(user, data, currentMessages[messageIndex], threadId);
      }
    }
    if (decryptedMessage !== undefined) {
      currentMessages[messageIndex].message = decryptedMessage;
...
async function migrateKeys(user, data, threadId) {
  const privateKey = data.ekPrivate;
  const publicKey = await importX25519PublicRaw(ub64(data.ekPublic));
...
export async function getWelcomeMessage(user, data, message, threadId) {
  await migrateKeys(user, data, threadId);
...
export async function sendDRDM2(user, data, threadId, message) {
  ...
  console.log("Chainkey s:", b64(chainKey_s));
  console.log("Chainkey r:", b64(chainKey_r));
...
export async function deriveNewKeys(user, data, data_b, threadId) {
  ...
  console.log("rootKey: ", b64(rootKey));
  console.log(
    "privateKey: ",
    b64(await crypto.subtle.exportKey("pkcs8", privateKey))
  );
  console.log("otherPublicKey_n: ", await exportKey(otherPublicKey_n));
  const dh = await runDH(privateKey, otherPublicKey_n);
  console.log("dh: ", b64(dh));
...
async function cycleDHRachet(user, data, message, threadId) {
  console.log(await getStoredKey(`otherPublicKey_n_${threadId}`));
  if (
    ...
    const { privateKey: newPrivateKey, publicKey: newPublicKey } = await generateX25519Keypair();
    ...
    console.log(b64(new Uint8Array(chainKey_r)));
