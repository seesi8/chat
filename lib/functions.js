...
import { getStorage, ref, uploadBytes, getDownloadURL, uploadString, getBytes } from "firebase/storage";
...
export const sendFileWithLock = async (threadId, file, user, data) => {
  const returnData = await drLock.acquire(threadId, async () => {
    return await sendFile(threadId, file, user, data, 0x03)
  });
...
const decryptDmMessage = async (currentMessage, user, data, threadId, thread, id) => {
  ...
  if (decryptedMessage == undefined) {
    state = await bypassMissed(n, pn, storedN, threadId, currentMessage);
  }
  ...
  if (decryptedMessage !== undefined) {
    if (!alreadyDecrypted && currentMessage.type == 0x01) {
...
function groupMessages(messages) {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (messages[i].sentBy.user == groupedMessages[groupedMessages.length - 1].sentBy.user && messages[i].read == groupedMessages[groupedMessages.length - 1].read && withinDistance(messages[i].timeSent, groupedMessages[groupedMessages.length - 1].timeSent)) {
...
export async function sendFileWithDHExchange(
  ...
  await storeKey(nextChainKey, `chainKey_${threadId}_s`);
  await storeKey(encrypted["nextChainKey"], `chainKey_${threadId}_s_raw`);
  await storeMessage(threadId, messageId, file);
  await storeMetadata(1, `nextN_${threadId}`);
  if (!restrict) {
    await storeMetadata(0, `PN_${threadId}`);
  }
