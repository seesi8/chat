...
export const decryptDmMessages = async (
  ...
  for (let messageIndex in currentMessages) {
    ...
    if (decryptedMessage == undefined) {
      if (
        currentMessages[messageIndex].header.n === 0 &&
        currentMessages[messageIndex].header.pn === 0 &&
        ...
        decryptedMessage = await decryptDRDM2(
          user,
          data,
          currentMessages[messageIndex],
          threadId
        ...
        } else if (currentMessages[messageIndex].header.n === 0) {
...
export const sendMessage = async (
  ...
  if (!dm) {
    ...
    if (n == null && pn == null) {
      console.log("crycle");
      sendDRDM2(user, data, threadId, message);
    } else {
      if (n == 0 && leader != user.uid) {
        console.log("crycle");
        sendDRDM2(user, data, threadId, message, n, pn, true);
      } else {
        sendDRDM(user, data, threadId, message);
      }
    }
  }
};
...
export async function sendDRDM(user, data, threadId, message) {
  const chainKey = await getStoredKey(`chainKey_${threadId}_s`);
  const publicKey_n = await getStoredKey(`publicKey_${threadId}_n`);
  const privateKey_n = await getStoredKey(`privateKey_${threadId}_n`);
  const n = await getStoredMetadata(`nextN_${threadId}`);
  let pn = await getStoredMetadata(`PN_${threadId}`);
  const messageId = uuidv4();
...
export async function decryptDRDM2(user, data, message, threadId) {
  const member = (
    await getDoc(doc(firestore, "users", message.sentBy.user))
  ).data();
...
export async function sendDRDM2(
  user,
  data,
  threadId,
  message,
