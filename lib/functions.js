...
export const sendMessage = async (threadId, message, user, data) => {
  ...
  if (!dm) {
    ...
    if (n == null && pn == null) {
      ...
      messageE = await sendDRDMWithDHExchange(user, data, threadId, message);
    } else {
      if (n == 0) {
        console.log("Send Cycled");
        ...
        messageE = await sendDRDM(user, data, threadId, message, n, pn);
      }
    }
  }
  return messageE;
...
export const sendMessageWithLock = async (threadId, message, user, data) => {
  const returnData = await drLock.acquire(threadId, async () => {
    return await sendMessage(threadId, message, user, data)
  });
...
const decryptDmMessage = async (currentMessage, user, data, threadId, thread, id) => {
  ...
  if (decryptedMessage !== undefined) {
    if (!alreadyDecrypted) {
      await sendMessage(threadId, "READ", user, data)
    }
...
export async function uploadMessage(chainKey, n, pn, publicKey_n, message, user, data, threadId) {
  const messageId = uuidv4();
...
export async function sendDRDM(user, data, threadId, message, n = 0, pn = 0) {
  ...
  const { nextChainKey, messageId } = await uploadMessage(chainKey, n, pn, publicKey_n, message, user, data, threadId)
