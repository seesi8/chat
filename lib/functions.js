...
export async function test(user, data) {
  const OPKPrivateKey = await getStoredKey("OPKPrivateKey")
  const SPKPrivateKey = await getStoredKey("SPKPrivateKey")
  ...
  for(let document of documents){
    ...
    const ekPrivate = await exportKey(await getStoredKey(`ekPrivate_${threadId}`), "pkcs8")
    const chainKeyS = await getStoredKey(`chainKey_${threadId}_s`)
    const chainKeyR = await getStoredKey(`chainKey_${threadId}_r`)
    const rootKey = await getStoredKey(`rootKey_${threadId}`)
    const otherPublicKey_n = await getStoredKey(`otherPublicKey_n_${threadId}`)
