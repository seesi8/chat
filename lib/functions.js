...
export const sendMessage = async (
  ...
  if (!dm) {
    ...
    if (n == null && pn == null) {
      console.log("crycle");
      sendDRDMWithDHExchange(user, data, threadId, message);
    } else {
      if (n == 0 && leader != user.uid) {
        console.log("crycle");
        sendDRDMWithDHExchange(user, data, threadId, message, n, pn, true);
      } else {
        sendDRDM(user, data, threadId, message, n, pn);
      }
    }
  }
};
...
export const decryptDmMessages = async (
  ...
  for (let messageIndex in currentMessages) {
    ...
    if (decryptedMessage == undefined) {
      ...
      console.log(n, storedN)
      console.log(pn, storedPN)
      if (n > storedN && storedPN == pn) {
        console.log("MISSED")
        console.log(n, storedN)
        ...
        storedN = n
        storedPN = pn
        await bypassMissedMessage(threadId, storedN)
      ...
      if (
        ...
        if (
          ...
          // UPDATE METADATA IF NOT LEADER

          const n_ = await getStoredMetadata(`nextN_${threadId}`);
          await storeMetadata(n_, `PN_${threadId}`);
...
export async function bypassMissedMessage(threadId, n) {

  const chainKey = await getStoredKey(`chainKey_${threadId}_r`);

  const { mkBytes,
    nextChainKey } = await skipMessageDR(
      chainKey,
    );

  const key = await importHKDFKeyRaw(nextChainKey);
  const mk = await importMessageKey(mkBytes);

  await storeKey(key, `chainKey_${threadId}_r`);
  await storeKey(nextChainKey, `chainKey_${threadId}_r_raw`);
  await storeKey(mk, `mk_${threadId}_${n}`);
  await storeKey(mkBytes, `mk_${threadId}_${n}_raw`);
}
...
export const createDRDM = async (user, data, members, groupName) => {
  ...
  const { result, opkIndex } = await x3dh(user, data, otherMember, groupId, members[1].uid);
...
export async function x3dh(user, data, data_b, threadId, uid_b) {
  ...
  console.log(ub64(preKey.key))
  console.log("sdjfndsj")
...
export async function sendWelcomeMessage(user, data, threadId, batch, ekPublic, ekPrivate) {
  const message = "Welcome";
  const messageId = uuidv4();

  const welcomeMessage = await getWelcomeMessage(user, data, message, threadId, ekPublic, ekPrivate);
...
export async function getWelcomeMessage(user, data, message, threadId, ekPublic, ekPrivate) {
...
export const sendMessageOutOfOrder = async (
  threadId,
  message,
  user,
  data,
...
export async function sendDRDMWithDHExchange(
  ...
  let member = (await getDoc(doc(firestore, "threads", threadId)))
    .data()
    .members.filter((item) => {
      return item != user.uid;
    })[0];
  let memberData = (await getDoc(doc(firestore, "users", member))).data();
...
export async function createBackup(user, data, passphrase) {
  ...
  const SPKPrivateKey = await exportKey(await getStoredKey("SPKPrivateKey"), "pkcs8")
  ...
  for (let document of documents) {
    ...
    if (await getStoredKey(`ekPrivate_${threadId}`) != undefined) {
      ekPrivate = await exportKey(await getStoredKey(`ekPrivate_${threadId}`), "pkcs8")
    }
    else {
      ekPrivate = null
    }
    ...
    const otherPublicKey_n = await exportKey(await getStoredKey(`otherPublicKey_n_${threadId}`))
    const publicKey_n = await exportKey(await getStoredKey(`publicKey_${threadId}_n`))
    const privateKey_n = await exportKey(await getStoredKey(`privateKey_${threadId}_n`), "pkcs8")
  ...
  const encrypted = await encryptObject(JSON.stringify({ threadsData, SPKPrivateKey, privateKey }), passphrase)
  return encrypted
}
...
export async function restoreBackup(userData, passphrase, setPopup) {
  ...
  try {
    ...
    const SPKPrivateKey = await importX25519PublicRaw(ub64(loaded["SPKPrivateKey"]), ["deriveBits", "deriveKey"], 'pkcs8')
    const privateKey = await importX25519PublicRaw(ub64(loaded["privateKey"]), ["deriveBits", "deriveKey"], 'pkcs8')

    console.log(userData.user.uid)
    for (let thread of loaded["threadsData"]) {
      const threadId = thread.threadId || "d36ceb77-5709-46d7-a0c7-27c46e2ae27c"
      ...
      const privateKey_n = await importX25519PublicRaw(ub64(thread.privateKey_n), ["deriveBits", "deriveKey"], 'pkcs8')
      const n = thread.n
      const pn = thread.pn
      const otherPublicKey_n = await importX25519PublicRaw(ub64(thread.otherPublicKey_n))
      ...
      if (thread.ekPrivate != null) {
        const ekPrivate = await importX25519PublicRaw(ub64(thread.ekPrivate), ["deriveBits", "deriveKey"], 'pkcs8')
...
export async function test(user, data) {
  ...
  await generateOPKS(user, data)
}
export async function testThread(user, data, threadId) {
  const n = await getStoredMetadata(`nextN_${threadId}`)
  const pn = await getStoredMetadata(`PN_${threadId}`)

  await sendMessageOutOfOrder(threadId, "OUT OF ORDER", user, data, n + 1, pn)
  console.log(n, pn)
  console.log(n + 1, pn)
}