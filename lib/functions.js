...
export async function createBackup(user, data, passphrase) {
  ...
  const encrypted = await encryptObject(JSON.stringify({ threadsData, OPKPrivateKey, SPKPrivateKey, privateKey }), passphrase)
  return encrypted
}
...
export async function test(user, data) {
  await storeAndDownloadBackup("hi", user, data, true)
  console.log(await restoreBackup({ user, data }, "hi"))
}