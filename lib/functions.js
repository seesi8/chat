...
export const sendTypeWithLock = async (threadId, message, user, data, type) => {
  const returnData = await drLock.acquire(threadId, async () => {
    return await sendMessage(threadId, message, user, data, type)
  });
...
const decryptDmMessage = async (currentMessage, user, data, threadId, thread, id) => {
  ...
  if (currentMessage.type == 0x01 || currentMessage.type == 0x02 || currentMessage.type == 0x05) {
  ...
  if (decryptedMessage !== undefined) {
    ...
    else if (currentMessage.type == 0x03 || currentMessage.type == 0x04) {
...
export const decryptDmMessages = async (
  ...
  for (let messageIndex in currentMessages) {
    ...
    if (decryptedMessage !== undefined) {
      ...
      else if (currentMessage.type == 0x01 || currentMessage.type == 0x05) {
        currentMessages[messageIndex].message = decryptedMessage;
...
function groupMessages(messages) {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (messages[i].sentBy.user == groupedMessages[groupedMessages.length - 1].sentBy.user && messages[i].type == groupedMessages[groupedMessages.length - 1].type && messages[i].read == groupedMessages[groupedMessages.length - 1].read && withinDistance(messages[i].timeSent, groupedMessages[groupedMessages.length - 1].timeSent) && messages[i].type != 0x05) {
