      ...
      if (decryptedMessage == undefined) {
        ...
        if (n > storedN && storedPN == pn) {
          ...
          for (let i = 0; i < n - storedN; i++) {
            await bypassMissedMessage(threadId, storedN + i);
          }
          storedN = n;
          storedPN = pn;
        }  
        if (n < storedN && storedPN == pn && n != 0) {
...
export async function decryptMissedDRDM(user, data, message, threadId, n) {
  const mk = await getStoredKey(`mk_${threadId}_${n}`);
...
export async function bypassMissedMessage(threadId, n) {
  let mk = await getStoredKey(`mk_${threadId}_${n}`);
...
export async function test(user, data) {
  const OUser = { uid: "UAaCCo5x9MTJivA67tXhGZ8Xz7q2" };
  const OData = (
    await getDoc(doc(firestore, "users", "UAaCCo5x9MTJivA67tXhGZ8Xz7q2"))
  ).data();
  if (NUM == 0) {
    ...
    threadId = await createDRDM(
      ...
      [
        ...
        { uid: "UAaCCo5x9MTJivA67tXhGZ8Xz7q2", username: "donte" },
      ],
      `TEST`,
      uuidv4()
    );
    ...
    // messageD2 = await deleteMessage(user, data, threadId, messageE2);
    // messageD3 = await deleteMessage(user, data, threadId, messageE3);
  } else {
    let messageE0 = await sendMessage(threadId, "Hello0", user, data);
    let messageE1 = await sendMessage(threadId, "Hello1", user, data);

    messageD2 = await deleteMessage(user, data, threadId, messageE0);
    messageD3 = await deleteMessage(user, data, threadId, messageE1);
  }
  NUM++;
