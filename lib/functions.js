...
export async function x3dh_r(user, data, data_b, salt) {
  salt = ub64(salt);
...
export async function sendWelcomeMessage(user, data, threadId, batch) {
  ...
  const welcomeMessage = await getWelcomeMessage(user, data, message);
...
export async function getWelcomeMessage(user, data, message) {
...
export async function decryptFirstDRDM(message, threadId) {
  const chainKey = await getStoredKey("chainKey");
  ...
  await storeKey(key, "chainKey")

  return decrypted.plaintext
}

export async function test(user, data) {
  // const result = await x3dh_r(user, data, member, salt);
}
