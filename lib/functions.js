...
export const sendMessage = async (
  ...
  if (!dm) {
    ...
    const pn = await getStoredMetadata(`nextN_${threadId}`)
    console.log(n, pn)
    if (n == null && pn == null) {
...
export async function decryptFirstDRDM(user, data, message, threadId) {
  ...
  const otherPublicKey = await importX25519PublicRaw(ub64(message.header.publicKey_n))
...
export async function sendDRDM2(user, data, threadId, message) {
  ...
  let member = (await getDoc(doc(firestore, "threads", threadId))).data().members.filter((item) => {
    return item != user.uid
  })[0]
  let memberData = (await getDoc(doc(firestore, "users", member))).data()
  memberData.uid = member
  const {chainKey, privateKey, publicKey, rootKey, salt} = await deriveNewKeys(user, data, memberData, threadId)
  console.log({chainKey, privateKey, publicKey, rootKey, salt})
}
