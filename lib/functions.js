      ...
      if (decryptedMessage == undefined) {
        if (n === 0 && pn === 0 && thread.leader != user.uid) {
          ...
          if (((await getStoredKey(`otherPublicKey_n_${threadId}`)) == null ||
            currentMessage.header.publicKey_n !=
            (await exportKey(await getStoredKey(`otherPublicKey_n_${threadId}`))))) {
            if (n != 0) {
              console.log("dsjkdgnfjkdgnfdg")
              await cycleDHRachet(
                user,
                data,
                currentMessage,
...
export async function bypassMissedMessageForce(threadId, n, publicKey_n, old = false) {
  let mk = await getMK(`mk_${threadId}_${n}_${publicKey_n}`);
  if (true) {
    console.log("BYPASSING");
    let chainKey;
...
export async function createBackup(user, data, passphrase) {
  ...
  for (let document of documents) {
    ...
    const chainKeyS_old = b64(await getStoredKey(`chainKey_${threadId}_s_raw_old`));
    const chainKeyR_old = b64(await getStoredKey(`chainKey_${threadId}_r_raw_old`));
...
export async function restoreBackup(userData, passphrase, setPopup) {
  ...
  try {
    ...
    for (let thread of loaded["threadsData"]) {
      ...
      const chainKeyR_old = await importHKDFKeyRaw(ub64(thread.chainKeyR_old));
      const chainKeyS_old = await importHKDFKeyRaw(ub64(thread.chainKeyS_old));
      ...
      await storeKey(chainKeyR_old, `chainKey_${threadId}_r_old`);
      await storeKey(ub64(thread.chainKeyR_old), `chainKey_${threadId}_r_raw_old`);
      await storeKey(chainKeyS_old, `chainKey_${threadId}_s_old`);
      await storeKey(ub64(thread.chainKeyS_old), `chainKey_${threadId}_s_raw_old`);
