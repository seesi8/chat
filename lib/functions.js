...
export const decryptDmMessages = async (
  ...
  for (let messageIndex in currentMessages) {
    ...
    const pns = await getStoredMetadata(`PN_${threadId}`);
    ...
    try {
      if (decryptedMessage == undefined) {
        ...
        if (ns != 0 && n != 0 && user.uid != thread.leader && ns != undefined && n != undefined) {
          console.log("Update Metadata Not Leader");
...
export async function bypassMissedMessage(threadId, n, publicKey_n, old = true) {
...
export async function sendDRDMWithDHExchange(
  ...
  await storeKey(chainKey_s_old, `chainKey_${threadId}_s`);
  await storeKey(chainKey_s_raw_old, `chainKey_${threadId}_s_raw`);
...
let messageD3;
let threadId;
const THEUID = "cjvz1CAxwwXXenx5EPYnzvtEqKI2";
const THEUSERNAME = "holiday";
// TESTING
