...
export const decryptDmMessages = async (
  ...
  let sendKey = await getDecryptedDmSendKey(1, user, data, threadId);
  let recvKey = await getDecryptedDmRecvKey(
    1,
    user,
    data,
    threadId,
  ...
  for (let messageIndex in currentMessages) {
    ...
    currentMessages[messageIndex].id = id;
    let decryptedMessage = "";
    const version = currentMessages[messageIndex].version;
    const sentBy = currentMessages[messageIndex].sentBy.user;
    if (version > currentVersion) {
      currentVersion = version;
      sendKey = await getDecryptedDmSendKey(version, user, data, threadId);
      recvKey = await getDecryptedDmRecvKey(
        version,
        user,
        data,
        threadId,
...
export async function x3dh(user, data, data_b) {
  console.log(data_b);
  ...
  await storeKey(chainKey, "chainKey");
  await storeKey(rootKey, "rootKey");
...
export const createDRDM = async (user, data, members, groupName) => {
  ...
  const result = await x3dh(user, data, otherMember);
...
export async function sendWelcomeMessage(user, data) {
  ...
  const chainKey = await getStoredKey("chainKey");
  ...
  await storeKey(nextChainKey, "chainKey");
